...
Deno.test("OPERAND_COUNT - maps operators to their operand counts", () => {
  ...
  assertEquals(
    OPERAND_COUNT.get("NOT"),
    1,
    "Operand count for 'NOT' should be 1",
  );
  assertEquals(
    OPERAND_COUNT.get("BETWEEN"),
    3,
    "Operand count for 'BETWEEN' should be 3",
  );
...
Deno.test("createTable - generates SQL for table creation with primary key", () => {
  const sql = createTable("users", { id: "SERIAL", name: "TEXT" }, {
    pk: "id",
  });
  assertEquals(
    sql,
...
Deno.test("createIndex - generates SQL for index with method", () => {
  const sql = createIndex("idx_users_name", "users", '"name"', {
    method: "BTREE",
  });
  assertEquals(
    sql,
...
Deno.test("setupTable - generates SQL for table setup with index", () => {
  ...
  const sql = setupTable("food", { code: "TEXT", name: "TEXT" }, rows, {
    pk: "code",
    index: true,
  });
  assertEquals(
...
Deno.test("setupTableIndex - generates SQL for basic index", () => {
  const sql = setupTableIndex("food", { code: "TEXT", name: "TEXT" }, {
    pk: "code",
    index: true,
  });
  assertEquals(
...
Deno.test("createTableData - generates SQL and data for table creation", () => {
  const { query, data } = createTableData("users", {
    id: "SERIAL",
    name: "TEXT",
  }, "id");
  assertEquals(
